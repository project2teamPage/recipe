...
document.addEventListener('DOMContentLoaded', function () {
    ...
    window.openReportModal = function(btn) {
        ...
        if (btn) {
            const type = btn.getAttribute('data-target-type') || 'COUMMUNITY_POST';
            const nickname = btn.getAttribute('data-target-nickname') || '';
            const id = btn.getAttribute('data-target-id') || '';

            if (targetTypeDisplay) {
                targetTypeDisplay.innerText = type === 'COMMUNITY_COMMENT' ? '커뮤니티 댓글' : '커뮤니티 게시글';
            }
            ...
            const type = reportBtn.getAttribute('data-target-type') || 'COMMUNITY_POST';
